...
self.addEventListener('push', (event) => {
  ...
  const options = {
    body: event.data ? event.data.text() : 'Water level alert from your IoT monitor',
...
self.addEventListener('message', (event) => {
  ...
  if (event.data && event.data.type === 'CACHE_WATER_DATA') {
    ...
    event.waitUntil(
      caches.open(CACHE_NAME)
        .then(cache => {
          return cache.put('/api/water-data', new Response(JSON.stringify(event.data.payload)));
        })
    );
  }
});

console.log('💧 Service Worker: Loaded and ready for IoT Water Tank Monitoring');